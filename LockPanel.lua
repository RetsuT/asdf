...
def("Master",true)
def("Hitbox",false) def("HitboxSize",10) def("HitboxTrans",1)
def("FireRate",false) def("FireCooldown",0) def("SnapPacket",false)
...
local function isHeld(b)
    ...
    local ok,kc=pcall(function() return Enum.KeyCode[b] end) return ok and kc~=nil and UIS:IsKeyDown(kc)
end
...
local function runPanel()
    ...
    applySpread()
    local function patchCD(tool,val) if not tool:IsA("Tool") then return end if type(tool:GetAttribute("ShootingCooldown"))~="number" then return end
        pcall(function() for _,conn in ipairs(getconnections(tool.Activated)) do local fn=conn.Function if type(fn)=="function" then for i,v in pairs(debug.getupvalues(fn)) do if type(v)=="number" and v>=0 and v<3 then debug.setupvalue(fn,i,val) end end end end end) end
    local function setFireRate(on) pcall(function() if type(rG._animAdjust)=="table" then genv.__origCB4=genv.__origCB4 or rG._animAdjust[4] rG._animAdjust[4]= on and function() end or genv.__origCB4 end end)
        local list={} for _,c in ipairs(LP.Backpack:GetChildren()) do list[#list+1]=c end if LP.Character then for _,c in ipairs(LP.Character:GetChildren()) do list[#list+1]=c end end
        for _,t in ipairs(list) do if t:IsA("Tool") then local cd=t:GetAttribute("ShootingCooldown") patchCD(t, on and C.FireCooldown or (cd or 0.15)) end end end
    setFireRate(C.Master and C.FireRate)
    LP.Backpack.ChildAdded:Connect(function(c) task.wait(0.1) if C.Master and C.FireRate then patchCD(c,C.FireCooldown) end end)
    LP.CharacterAdded:Connect(function(ch) ch.ChildAdded:Connect(function(c) task.wait(0.1) if C.Master and C.FireRate then patchCD(c,C.FireCooldown) end end) task.wait(1) setFireRate(C.Master and C.FireRate) end)
    ...
    local function visible(pos,tc) local Cam=workspace.CurrentCamera if not Cam then return true end
        ...
        rp.FilterDescendantsInstances={LP.Character, workspace:FindFirstChild("Ignored"), tc}
        return workspace:Raycast(Cam.CFrame.Position, pos-Cam.CFrame.Position, rp)==nil end
    local function nearestChar(radius) local Cam=workspace.CurrentCamera if not Cam then return end
        local ml=UIS:GetMouseLocation()-GuiService:GetGuiInset() local best,bestD=nil,radius
        for _,p in ipairs(Players:GetPlayers()) do if p~=LP and p.Character then local char=p.Character
            if not (C.KnockCheck and knocked(char)) then local hrp=char:FindFirstChild("HumanoidRootPart") local hum=char:FindFirstChildOfClass("Humanoid")
                if hrp and hum and hum.Health>0 then local sp,on=Cam:WorldToViewportPoint(hrp.Position) if on and sp.Z>0 then local d=(Vector2.new(sp.X,sp.Y)-ml).Magnitude if d<bestD then bestD=d best=char end end end end end end
        return best end
    -- TRIGGERBOT: raycast through crosshair (exact) + FOV multiplier fallback
    ...
    genv.__trigConn=RunService.Heartbeat:Connect(function()
        ...
        local mp=UIS:GetMouseLocation()-GuiService:GetGuiInset()
        local fire=false
        local ray=Cam:ViewportPointToRay(mp.X,mp.Y)
        local rp=RaycastParams.new() rp.FilterType=Enum.RaycastFilterType.Exclude rp.FilterDescendantsInstances={ch, workspace:FindFirstChild("Ignored")}
        local res=workspace:Raycast(ray.Origin, ray.Direction*3000, rp)
        if res then
        ...
        if not fire then
            local radius=30*C.TriggerMult
            for _,p in ipairs(Players:GetPlayers()) do
                if p~=LP and p.Character and not (C.KnockCheck and knocked(p.Character)) then
                    local char=p.Character
                    for _,pn in ipairs({"Head","UpperTorso","Torso"}) do
                        local part=char:FindFirstChild(pn)
                        if part then
                            local sp,on=Cam:WorldToViewportPoint(part.Position)
                            if on and sp.Z>0 and (Vector2.new(sp.X,sp.Y)-mp).Magnitude<=radius and ((not C.WallCheck) or visible(part.Position,char)) then fire=true break end
                        end
                    end
                end
    ...
    -- SAUCE: horizontal mouse-lock to body column; vertical stays free
    ...
    genv.__sauceConn=RunService.RenderStepped:Connect(function(dt)
        ...
        local root=char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso") or char:FindFirstChild("HumanoidRootPart") if not root then return end
        if C.WallCheck and not visible(root.Position,char) then return end
        local sp,on=Cam:WorldToViewportPoint(root.Position) if not on or sp.Z<=0 then return end
        local targetX=sp.X+GuiService:GetGuiInset().X
        if C.Shake then targetX=targetX+math.sin(tick()*7)*1.5 end
        local dx=targetX-UIS:GetMouseLocation().X
        local alpha=math.clamp((dt*60)/math.max(C.Smoothness,1),0,1)
        if mousemoverel then pcall(mousemoverel,dx*alpha,0) end
    end)
    -- SHOT DETECTION
    ...
    -- THROTTLED LOOP: hitbox + esp + spectate + config-save
    local origSize=setmetatable({},{__mode="k"})
    local function enlarge() local sz=Vector3.new(C.HitboxSize,C.HitboxSize,C.HitboxSize) for _,p in ipairs(Players:GetPlayers()) do if p~=LP and p.Character then local h=p.Character:FindFirstChild("Head")
        if h and h:IsA("BasePart") then if origSize[h]==nil then origSize[h]=h.Size end
            if h.Size~=sz then h.Size=sz end if h.Transparency~=C.HitboxTrans then h.Transparency=C.HitboxTrans end
            if h.CanCollide then h.CanCollide=false end if not h.CanQuery then h.CanQuery=true end if not h.Massless then h.Massless=true end end end end end
    local function restoreHeads() for h,s in pairs(origSize) do if h and h.Parent then pcall(function() h.Size=s h.Transparency=0 end) end end table.clear(origSize) end
    ...
    for _,n in ipairs({"__hbConn","__espConn","__specConn","__slowConn"}) do if genv[n] then pcall(function() genv[n]:Disconnect() end) genv[n]=nil end end
    local acc=0
    genv.__slowConn=RunService.Heartbeat:Connect(function(dt) acc=acc+dt if acc<0.06 then return end acc=0
        if C.Master and C.Hitbox then enlarge() elseif next(origSize) then restoreHeads() end updESP() updSpec()
        local s pcall(function() s=HttpService:JSONEncode(C) end)
        if s and s~=genv.__lastCfg then genv.__lastCfg=s if writefile then pcall(writefile,"LockPanelConfig.json",s) end end
    end)
    ...
    local function mkToggle(parent,order,labelText,default,cb)
        ...
        new("TextButton",{Size=UDim2.fromScale(1,1),BackgroundTransparency=1,Text="",Parent=row}).MouseButton1Click:Connect(function() set(not st) cb(st) end)
    ...
    local function mkSlider(parent,order,labelText,minv,maxv,default,suffix,cb,divisor) divisor=divisor or 1
        ...
        local dragging=false local function setv(px) local r=math.clamp((px-track.AbsolutePosition.X)/track.AbsoluteSize.X,0,1) local v=math.floor(minv+(maxv-minv)*r+0.5) r=(v-minv)/(maxv-minv) fill.Size=UDim2.fromScale(r,1) knob.Position=UDim2.new(r,-7,0.5,-7) val.Text=(v/divisor)..suffix cb(v/divisor) end
        local function down(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dragging=true setv(i.Position.X) end end
        track.InputBegan:Connect(down) knob.InputBegan:Connect(down)
        UIS.InputChanged:Connect(function(i) if dragging and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then setv(i.Position.X) end end)
    ...
    local function mkKeybind(parent,order,labelText,getBind,cb)
        ...
        local listening=false kb.MouseButton1Click:Connect(function() listening=true kb.Text="..." end)
        UIS.InputBegan:Connect(function(i) if not listening then return end local name
            if i.UserInputType==Enum.UserInputType.Keyboard then name=i.KeyCode.Name elseif i.UserInputType==Enum.UserInputType.MouseButton1 then name="MouseButton1" elseif i.UserInputType==Enum.UserInputType.MouseButton2 then name="MouseButton2" elseif i.UserInputType==Enum.UserInputType.MouseButton3 then name="MouseButton3" end
            if name then listening=false kb.Text=friendly(name) cb(name) end end)
    end
    ...
    local function mkDropdown(parent,order,labelText,getCur,setCur)
        ...
        btn.MouseButton1Click:Connect(function() openPicker(function(name) setCur(name=="None" and "" or name) btn.Text=(getCur()~="" and getCur() or "None") end) end)
    end

    local snapSet
    local function needSnap() if not C.SnapPacket then C.SnapPacket=true if snapSet then snapSet(true) end end end
    mkToggle(body,1,"Master Enable",C.Master,function(s) C.Master=s applySpread() setFireRate(C.Master and C.FireRate) notify("Master "..(s and "ON" or "OFF")) end)
    ...
    mkToggle(cP,1,"Big Hitboxes",C.Hitbox,function(s) C.Hitbox=s if s then needSnap() end end)
    mkSlider(cP,2,"Size",1,30,C.HitboxSize,"",function(v) C.HitboxSize=v end)
    mkSlider(cP,3,"Invisibility",0,100,math.floor((C.HitboxTrans or 1)*100),"%",function(v) C.HitboxTrans=v/100 end)
    mkToggle(cP,4,"Rapid Fire",C.FireRate,function(s) C.FireRate=s setFireRate(C.Master and s) end)
    mkSlider(cP,5,"Cooldown",0,500,math.floor((C.FireCooldown or 0)*1000)," ms",function(v) C.FireCooldown=v/1000 setFireRate(C.Master and C.FireRate) end)
    snapSet=mkToggle(cP,6,"Server Hit Snap",C.SnapPacket,function(s) C.SnapPacket=s end)
    mkToggle(cP,7,"Long Range",C.RangeOn,function(s) C.RangeOn=s end)
    mkSlider(cP,8,"Range",50,5000,C.RangeOverride," studs",function(v) C.RangeOverride=v end)
    mkToggle(cP,9,"Custom Spread",C.SpreadOn,function(s) C.SpreadOn=s applySpread() end)
    mkSlider(cP,10,"Spread %",0,100,math.floor((C.SpreadFactor or 0)*100),"%",function(v) C.SpreadFactor=v/100 applySpread() end)
    ...
    selTab("COMBAT")
    local dragA,dS,sPos=false local function bd(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dragA=true dS=i.Position sPos=win.Position end end
    header.InputBegan:Connect(bd) title.InputBegan:Connect(bd)
    UIS.InputChanged:Connect(function(i) if dragA and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then local d=i.Position-dS win.Position=UDim2.new(sPos.X.Scale,sPos.X.Offset+d.X,sPos.Y.Scale,sPos.Y.Offset+d.Y) end end)
    UIS.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dragA=false end end)
    UIS.InputBegan:Connect(function(i,gpe) if gpe then return end if i.KeyCode==Enum.KeyCode.RightShift then gui.Enabled=not gui.Enabled end end)
