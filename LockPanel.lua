...
local function runPanel()
    ...
    genv.__slowConn=RunService.Heartbeat:Connect(function(dt)
        ...
        if sacc>=2 then sacc=0
            local parts={} for k,v in pairs(C) do parts[#parts+1]=tostring(k).."="..tostring(v) end table.sort(parts)
            local sig=table.concat(parts,"|")
            if sig~=genv.__lastCfg then genv.__lastCfg=sig if writefile then pcall(function() writefile("LockPanelConfig.json",HttpService:JSONEncode(C)) end) end end
        end
    ...
    local function mkSlider(parent,order,labelText,minv,maxv,default,suffix,cb,divisor) divisor=divisor or 1
        ...
        local function setv(px) local r=math.clamp((px-track.AbsolutePosition.X)/track.AbsoluteSize.X,0,1) local v=math.floor(minv+(maxv-minv)*r+0.5) r=(v-minv)/(maxv-minv) fill.Size=UDim2.fromScale(r,1) knob.Position=UDim2.new(r,-7,0.5,-7) val.Text=(v/divisor)..suffix cb(v/divisor) end
    ...
    local function mkKeybind(parent,order,labelText,getBind,cb)
        ...
        kb.MouseButton1Click:Connect(function() kb.Text="..." activeBind=function(name) kb.Text=friendly(name) cb(name) end end)
    end
